function delta = FindDeltaWaves(filtered, filtered_fast, fs,varargin)
...
peak = zeros(size(delta, 1), 1);
y = zscore(filtered_fast(:,2));
for i = 1:size(delta, 1)
    peak(i) = max(y(round(delta(i,1)*fs) : round(delta(i,3)*fs)));
end

% figure
% plot(filtered(:,1), filtered(:,2), 'k')
% hold on
% plot(filtered_fast(:,1), filtered_fast(:,2), 'b')
% plot(delta(:,1), filtered(round(delta(:,1)*fs),2), '>k', 'MarkerFaceColor', 'k')
% plot(delta(:,2), filtered(round(delta(:,2)*fs),2), '^k', 'MarkerFaceColor', 'k')
% plot(delta(:,3), filtered(round(delta(:,3)*fs),2), '<k', 'MarkerFaceColor', 'k')
